...
운영 시스템 자체는 이 공개 저장소에 포함되지 않습니다. 이 문서는 익명화·정제된 정보만으로 기술적 문제, 개발 과정의 도전 과제, 아키텍처, 구현 방식, 배운 점을 설명합니다. 공개 저장소는 합성 샘플 데이터를 사용해 동일한 엔지니어링(집계 엔진·API·대시보드)을 재구성한 포트폴리오 버전입니다.
...
공개 저장소는 동일한 개념을 로컬 전용 합성 데이터로 재구성합니다. 운영 설비 인터페이스, 운영 인증, 운영 인프라, 고객별 구현은 포함하지 않습니다.
...
**롤업 집계 엔진** — 이 프로젝트의 핵심입니다. 설비에서 수집되는 누적 카운터(`RunTime`/`CutTime`)를 연속 값의 델타로 변환하고, 카운터 리셋(음수 델타), 10분 초과 유휴 갭, 비정상 점프(스텝 상한 120초)를 보정해 KST `(설비, 날짜, 시)` 버킷으로 집계합니다. 스케줄러(운영, 최근 윈도우)와 백필 러너(히스토리 1회 재집계)가 동일한 서비스/리포지토리 경로를 공유하며, unique 인덱스 기반 upsert로 재집계가 멱등하게 동작합니다. 델타 보정 규칙은 순수 함수(`DeltaRule`)로 분리해 DB 없이 단위 테스트합니다. 상세는 [ROLLUP_ARCHITECTURE.md](ROLLUP_ARCHITECTURE.md) 참고.

**백엔드 (Spring Boot)** — 읽기 중심 대시보드 엔드포인트가 프론트엔드용 데이터 구조를 반환합니다. MongoDB 조회, 설비 운영 데이터 집계, 가동률·RunTime/CutTime 지표 준비, 알람 이력 요약, 상태 분포 데이터, 차트/테이블용 응답 형태 가공을 담당하며, 대시보드는 원천 신호가 아니라 롤업이 만든 요약 컬렉션을 읽습니다.
...
제조 데이터 구조 및 대시보드 요구사항 분석, 현장 요구사항 논의를 통한 핵심 설비 지표 선정, MongoDB 쿼리·집계 로직 설계, 요약 데이터 구조 설계, 스케줄 집계 구현, Spring Boot API 구현, React 대시보드 및 차트 시각화 구현, 필터 구현, 로컬 런타임 테스트 및 배포 워크플로 지원, 아키텍처·데이터 흐름·보안 경계 문서화, 합성 데이터 기반 공개 포트폴리오 재구성을 수행했습니다.
...
- 런타임 스모크 테스트와 문서화는 기술 포트폴리오의 신뢰도를 높인다.
