...
This repository uses only synthetic demo data. The data files described here are not copied from any production system and must not contain real customer names, site names, equipment names, LOT numbers, alarm records, user information, credentials, server addresses, logs, or screenshots.

The backend API and frontend charts will be implemented against this schema.
